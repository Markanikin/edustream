...
function App() {
        ...
        contents: [
          {
            ...
            parts: [
              {
                text: `Тема: ${input}. Інструкція: ${instructions[level]}. Мова: українська. Виділяй ключові моменти жирним.`,
              },
            ],
          },
        ],
          ...
          contents: [
            {
              ...
              parts: [
                {
                  text: `Дай коротку назву (макс 3 слова) для теми: "${input}". Тільки назва.`,
                },
              ],
            },
          ],
  ...
  return (
    <div className="min-h-screen bg-[#02040a] text-slate-100 font-sans selection:bg-cyan-500/40 overflow-x-hidden relative flex">
      <aside
        className={`fixed top-0 left-0 h-full bg-[#050810]/98 backdrop-blur-2xl border-r border-cyan-500/10 transition-all duration-500 z-[100] ${
          isSidebarOpen ? "w-full sm:w-72" : "w-0 overflow-hidden"
        }`}
      >
        <div className="p-6 w-full sm:w-72">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-cyan-500 font-black text-[10px] uppercase tracking-[0.3em]">
              Історія запитів
            </h3>
            <button
              onClick={() => setIsSidebarOpen(false)}
              className="sm:hidden text-cyan-500 text-xl"
            >
              ✕
            </button>
          </div>
          <div className="space-y-4">
            {history.map((item) => (
              ...
              >
                <p className="text-sm font-bold text-slate-300 group-hover:text-cyan-400 transition-colors truncate">
                  {item.title}
                </p>
          ...
          <button
            onClick={() => {
              setHistory([]);
              localStorage.removeItem("eduMindHistory");
            }}
        ...
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10 flex flex-col min-h-screen">
          <header className="flex justify-between items-center mb-12 md:mb-20">
            <div className="flex items-center gap-4 md:gap-6">
              ...
              >
                <div
                  className={`h-0.5 bg-cyan-500 transition-all ${
                    isSidebarOpen
              ...
              <div className="flex items-center gap-2 md:gap-4 group">
                <div className="w-10 h-10 md:w-14 md:h-14 bg-transparent border-2 border-cyan-500 rounded-xl md:rounded-2xl flex items-center justify-center shadow-[0_0_15px_rgba(6,182,212,0.4)]">
                  <span className="text-xl md:text-3xl font-black text-cyan-400 italic">
                    E
                  </span>
                </div>
                <h1 className="text-lg md:text-2xl font-black tracking-[0.1em] md:tracking-[0.2em] text-white uppercase">
                  E-Mind
                </h1>
              </div>
            </div>
            ...
            <a
              ...
              target="_blank"
              className="hidden sm:block px-6 md:px-8 py-3 font-black text-cyan-400 border border-cyan-500/30 rounded-full hover:bg-cyan-500/10 transition-all backdrop-blur-md uppercase text-[10px] tracking-[0.2em]"
            >
              Support
            </a>
          </header>
          ...
          <main className="flex-grow flex flex-col items-center">
            <div className="text-center mb-8 md:mb-16 space-y-4 md:space-y-6 animate-in fade-in zoom-in duration-1000">
              ...
              <p className="text-slate-500 text-xs md:text-lg uppercase tracking-[0.2em] md:tracking-[0.4em] font-light px-4">
                Доступ до знань нового покоління
              </p>
              ...
              <div className="flex flex-wrap justify-center bg-black/40 p-1 rounded-2xl border border-white/10 backdrop-blur-3xl shadow-xl">
                {["child", "student", "pro"].map((id) => (
                  <button
                    key={id}
                    onClick={() => setLevel(id)}
                    className={`px-4 md:px-8 py-2 md:py-2.5 rounded-xl text-[10px] md:text-xs font-black tracking-widest uppercase transition-all duration-500 ${
                      level === id
            ...
            <div className="w-full max-w-3xl relative px-2">
              <div className="group relative rounded-3xl md:rounded-[2.5rem] bg-black/60 border border-cyan-500/20 focus-within:border-cyan-500/60 transition-all duration-500">
                <div className="flex flex-col md:flex-row gap-2 p-2 md:p-3">
                  ...
                  <button
                    onClick={() => handleAsk()}
                    disabled={loading}
                    className="bg-transparent border-2 border-cyan-500 text-cyan-400 px-6 md:px-12 py-4 md:py-5 rounded-2xl md:rounded-[1.8rem] font-black text-sm md:text-lg hover:bg-cyan-500 hover:text-black transition-all"
                  >
              ...
              {error && (
                <p className="mt-4 text-red-500 text-center font-mono text-[9px] md:text-[10px] uppercase tracking-[0.3em] animate-pulse">
                  {error}
                </p>
              )}
            </div>
            ...
            {result && (
              <div className="mt-8 md:mt-16 w-full max-w-4xl bg-[#050810]/80 border border-cyan-500/10 rounded-3xl p-6 md:p-10 backdrop-blur-3xl shadow-2xl relative text-left">
                ...
                <button
                  ...
                  className="absolute top-4 md:top-8 right-6 md:right-8 text-cyan-500 text-[9px] md:text-[10px] font-black tracking-widest uppercase border-b border-cyan-500/20"
                >
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 md:gap-10">
              <div className="flex flex-col items-center md:items-start space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] md:text-[11px] font-black text-green-500 uppercase tracking-[0.2em]">
                    СИСТЕМА АКТИВОВАНА
                  </span>
                </div>
              ...
              <div className="text-right text-[9px] md:text-[10px] text-slate-600 font-mono uppercase tracking-widest text-center md:text-right">
                ...
                <span className="text-cyan-800 font-black italic">
